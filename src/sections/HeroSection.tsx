 ...
 * needs no scrolling and no clicking to reach. The two scroll CTAs are the
 * only client-side code here; the card itself is server-rendered.
 */
export function HeroSection() {
  return (
    <section
      ...
      className="flex min-h-[88svh] scroll-mt-24 items-center px-6 py-24 sm:px-10"
    >
      <div className="mx-auto w-full max-w-3xl rounded-2xl border border-[var(--border-soft)] bg-[var(--surface)] p-8 backdrop-blur-md sm:p-12">
        ...
        <h1
          id="hero-heading"
          className="mt-7 font-[family-name:var(--font-display)] text-[clamp(2.75rem,8vw,4.5rem)] leading-[1] tracking-[-0.02em] text-[var(--ink)]"
        >
          {heroContent.name}
        </h1>

        <p className="mt-4 text-xl text-[var(--ink)] sm:text-2xl">{heroContent.role}</p>

        <p className="mt-4 max-w-xl text-[17px] leading-relaxed text-[var(--ink-muted)]">
          {heroContent.tagline}
        </p>

        <ul className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-3 border-t border-[var(--border-soft)] pt-7 text-[15px]">
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3">
          ...
          <a
            ...
            className="flex items-center gap-2 rounded-lg px-3 py-2.5 text-[15px] text-[var(--ink-muted)] transition-colors hover:text-[var(--ink)]"
          >
